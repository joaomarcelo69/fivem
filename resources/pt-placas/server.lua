...
local function plate_exists(plate)
  ...
  if MySQL and (MySQL.scalar or MySQL.query) then
    if MySQL.scalar then
      ...
      local rows = MySQL.query('SELECT 1 FROM vehicle_plates WHERE plate = ? LIMIT 1', { plate })
      return rows and rows[1] ~= nil
    end
  elseif exports and exports.oxmysql and exports.oxmysql.scalar then
    local res = exports.oxmysql:scalar('SELECT 1 FROM vehicle_plates WHERE plate = ? LIMIT 1', { plate })
    return res == 1
  else
    return false
  end
...
exports('ReservePlate', function(plate, owner_cid, model)
  ...
  if MySQL and MySQL.insert then
    local id = MySQL.insert(sql, { plate, owner_cid, model })
    ok = id ~= nil
  elseif exports and exports.oxmysql and exports.oxmysql.insert then
    local done = false
    exports.oxmysql:insert(sql, { plate, owner_cid, model }, function(id)
      done = id ~= nil
    end)
    ok = true -- assume ok na via assíncrona
  else
...
exports('GeneratePlate', function(owner_cid, model)
  ...
  -- fallback: permite duplicada em último caso
  local plate = format_plate()
  exports['pt-placas']:ReservePlate(plate, owner_cid, model)
  return plate
end)

-- Comando para admins gerarem placa rapidamente
