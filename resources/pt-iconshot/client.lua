...
local function captureModel(model, fileName)
  ...
  -- Ajeitar o prop (alinhamento básico)
  ...
  -- Fundo e iluminação: céu limpo estilo estúdio
  ...
  -- HUD off
  DisplayRadar(false)
  -- Desativar efeitos para nitidez
  SetUseHiDof()
  SetTimecycleModifier('')

  -- Criar um plano de chão simples branco para dar sombra/silhueta (usa uma prop leve)
  ...
  if IsModelInCdimage(groundHash) then
    ...
    if ground ~= 0 then
      ...
      SetEntityAlpha(ground, 200, false) -- quase branco translúcido
  ...
  -- Pedir screenshot ao servidor
  TriggerServerEvent('pt-iconshot:capture', fileName)

  -- Esperar confirmação do servidor (simples)
  local ok = false
  local done = false
