...
CreateThread(function()
  ensureSchema()
  -- Best-effort migration: ensure unique key for upsert works even if table existed before
  if hasMySQL() then
    pcall(function()
...
exports('AddItemForPlayer', function(sourceId, item, count)
  ...
  -- Fallback DB upsert if we have identifier
  if hasMySQL() then
...
-- Add support for direct insert by citizenid with UPSERT semantics
