...
-- helper: get docs from pt-vehicle-docs table directly
...
AddEventHandler('pt-inspections:server:check', function(plate)
  ...
  -- táxi licença: simples (se job não for taxi/driver e o veículo for classe táxi - aqui stub sempre true)
  local pdata = P.PlayerData
  local isTaxi = pdata and pdata.job and (pdata.job.name == 'taxi')
  -- stub check: aplica multa se não for taxi mas estiver a operar em paragem taxi (já acionou check)
  if not isTaxi then
