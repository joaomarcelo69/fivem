...
-- Simple in-memory government ledger; could be persisted via DB later
local Government = { balance = 0 }
local Debts = { byCitizen = {} } -- { [citizenid] = { { id, type, amount, reason, createdAt, paid=false } } }
...
-- Property taxes (simplified for gameplay)
...
-- Apply IVA to a purchase total, returning net paid and IVA collected
...
-- When prices are gross (contain VAT), compute IVA portion: total * rate / (1+rate)
...
-- Apply IRS withholding for a payment to a player
