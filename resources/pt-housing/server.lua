...
--   [propertyId] = {
--     owner = citizenid,
--     tenants = { [citizenid]=expiry },
--     keys = { [citizenid]=true }, -- acessos (chaves) sem prazo
--     listed = { type='sale'|'rent', price=number },
--     pendingIncome = 0 -- receita pendente por vendas/rendas
--   }
-- }
...
-- Export: obter mailbox de uma propriedade por id
...
-- Export: propriedades do jogador
...
-- Comprar propriedade (se listada ou preço base)
...
AddEventHandler('pt-housing:buy', function(propertyId)
  ...
  -- Creditar venda ao antigo dono (se aplicável)
  ...
  -- Definir mailbox do cidadão para a mailbox desta propriedade (se existir pt-commerce)
...
-- Callback: listar propriedades com estado
...
-- Export para decoração: pode decorar se for dono ou inquilino válido e perto da propriedade
...
-- Export utilitário: verificar acesso (owner/keys/tenant)
...
-- Limpeza periódica de arrendamentos expirados
...
-- Notificar receitas pendentes ao entrar
