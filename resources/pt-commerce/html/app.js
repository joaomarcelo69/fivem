...
function closeShop(){
  ...
  fetch(`https://${GetParentResourceName()}/ui:closeFocus`, { method: 'POST', body: '{}' });
}
  ...
  (orders||[]).forEach(o => {
    ...
    row.querySelectorAll('button').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        fetch(`https://${GetParentResourceName()}/ctt:${act}`, { method:'POST', body: JSON.stringify({ id }) });
      });
    });
...
checkoutBtn.addEventListener('click', () => {
  ...
  fetch(`https://${GetParentResourceName()}/shop:checkout`, { method:'POST', body: JSON.stringify({ items, method }) });
});
  ...
  if(filterBar){
    ...
    const first = filterBar.querySelector('[data-cat="all"]');
      ...
      pinConfirm.addEventListener('click', () => {
        ...
        fetch(`https://${GetParentResourceName()}/ctt:pin`, { method:'POST', body: JSON.stringify({ id: pinOrderId, pin: v }) });
        closePin();
      });
