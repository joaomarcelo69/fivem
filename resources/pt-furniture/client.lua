...
CreateThread(function()
  while true do
    ...
    if placing and DoesEntityExist(preview) then
      ...
      -- snapping da posição (grelha ~ 0.05m precisão, 0.25m default)
      local grid = IsControlPressed(0, 21) and 0.05 or 0.25 -- Shift para precisão
      if snapEnabled then
      ...
      -- rotação: Q/E e setas esquerda/direita, passo fino com Shift
      local step = IsControlPressed(0, 21) and 0.5 or 2.5
      if IsControlPressed(0, 44) or IsControlPressed(0, 174) then -- Q ou Left
        SetEntityHeading(preview, GetEntityHeading(preview) - step)
      elseif IsControlPressed(0, 38) or IsControlPressed(0, 175) then -- E ou Right
        SetEntityHeading(preview, GetEntityHeading(preview) + step)
      end

      -- altura: setas cima/baixo
      ...
      -- alinhamento ao chão (G)
      if IsControlJustPressed(0, 47) and groundZ then -- G
      ...
      -- verificação básica de colocação: precisa de chão próximo e dentro da distância de colocação
      ...
      -- feedback visual (marcador verde/vermelho)
      ...
      -- confirmar com ENTER (apenas se válido), cancelar com BACKSPACE
      if IsControlJustPressed(0, 18) and valid then -- Enter
...
-- Comandos simples para testar: /place <item>, /fmove, /fremove (olhando para o objeto)
...
-- Tornar itens utilizáveis na mão iniciando colocação
...
CreateThread(function()
  ...
  -- fallback simples via callback do qb
