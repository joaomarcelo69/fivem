...
local Warehouse = loadJson(SAVE_FILE) -- { sku => count }
...
-- Imports: simulate arrivals at hubs, crediting warehouse stock
...
-- Create delivery route to a shop for given product; consumes warehouse stock and yields cash to driver
...
-- Delivery complete: restock shop using pt-shops export and pay driver
...
AddEventHandler('pt-logistics:deliver', function(payload)
  ...
  if GetResourceState('pt-shops') ~= 'started' then
    ...
    addWarehouse(sku, units) -- rollback
    return
  end
  -- Apply yields to target shop stock
  for _, y in ipairs(product.yields or {}) do
    local total = math.floor((y.count or 0) * (units / (LogiConfig.VehicleCapacity.van))) -- scale by van units
    if total > 0 then
      pcall(function() exports['pt-shops']:RestockShop(shopId, y.item, total) end)
    end
  end
  -- Pay driver for route (flat + per unit)
  local basePay = 250
  local perUnit = 0.5
...
-- Send shop coords for waypoint (uses pt-shops server registry)
...
-- Dashboard: list shops by time since last delivery to help manual scouting when no GPS
...
-- NPC auto-restock: runs periodically, checks shops with low stock or long time since last delivery, and restocks from warehouse
CreateThread(function()
  while true do
    ...
    for _, s in ipairs(shops or {}) do
      ...
      if needs then
        -- find first SKU with available warehouse units
        for _, p in ipairs(products) do
