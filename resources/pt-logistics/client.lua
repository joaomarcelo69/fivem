...
-- Simple interaction loops at hubs and warehouse
CreateThread(function()
  while true do
    Wait(0)
    ...
    -- Warehouse: create delivery route to nearest shop blip via UI-less simple flow
...
-- When a route is created, show waypoint and hold E at shop to deliver
RegisterNetEvent('pt-logistics:route', function(route)
  local shopId = route.shopId
  -- GPS requirement: check if player has a gps item
  local hasGPS = false
  ...
  if hasGPS then
    -- try to get shop coords from server (pt-shops export not directly on client)
  ...
  CreateThread(function()
    ...
    while not delivered do
      Wait(0)
      -- se conhecemos a doca, exigir proximidade
      if route.dock then
...
-- Receive shop coords and set waypoint (if GPS present)
...
-- Simple dashboard: press F8 to print shops sorted by time since last delivery (as a guide without GPS)
