...
-- Tabela: vehicle_docs (plate PK, owner citizenid, insurance_exp INT, inspection_exp INT)
...
-- Persist props snapshot per plate to reconstruct vehicle on release/valet
local VehicleProps = {}

-- Allow other scripts to save props at impound time if available
...
-- Comandos para registar/renovar
...
-- Pedido STOP: polícia solicita documentos de um jogador próximo
...
AddEventHandler('pt-vehicle-docs:requestDocs', function(targetSrc)
  ...
  -- Multa automática quando inválidos (pode ajustar valores na config)
  if d then
...
-- Callback: obter documentos para um target (por source id)
...
-- Impound API
-- 1) From STOP: validate docs; if invalid, record impound and return ok + plate
QBCore.Functions.CreateCallback('pt-vehicle-docs:impoundIfInvalid', function(source, cb, targetSrc)
  ...
  -- snapshot props if vehicle entity is available via source (client should emit props separately for reliability)
  -- Auto fine
  if d then
...
-- 2) Release command (owner or police can release with fee)
RegisterCommand('levantarapreensao', function(source, args)
  ...
  -- Oferecer opção de entrega por condutor (valet) em vez de spawn imediato
  TriggerClientEvent('pt-vehicle-docs:client:offerValet', src, plate)
end)

-- Iniciar valet: envia ao cliente dados para spawn e condução
...
AddEventHandler('pt-vehicle-docs:server:startValet', function(plate)
  ...
  -- garantir que não está apreendido
  ...
  -- Email no telemóvel a avisar que o carro está a caminho
  local P = QBCore.Functions.GetPlayer(src)
