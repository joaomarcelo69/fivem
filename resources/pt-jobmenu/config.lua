...
-- Payouts configuráveis por job (usados no servidor)
...
-- Opcional: lista de modelos permitidos para polícia
...
-- EMS: se o servidor tiver um script médico que já faz revive, desligar aqui e adaptar chamadas
EMSConfig = EMSConfig or {
  enableBasicRevive = true,
  enableBasicStabilize = true,
}

-- Tecla para alternar serviço nos blips (38 = E)
DutyInteractKey = DutyInteractKey or 38
DutyInteractName = DutyInteractName or 'E'

-- Garagens por emprego (opcional)
...
-- Zonas de limite de velocidade (simples): usa o limite mais próximo dentro do raio; caso contrário aplica limite genérico
SpeedLimits = SpeedLimits or {
  { coords = vector3(250.0, -1000.0, 28.0), radius = 1000.0, limit = 50 },   -- Urbana densa (MRPD, Pillbox, Legion)
  { coords = vector3(380.0, -1600.0, 29.0), radius = 800.0,  limit = 50 },   -- Zona Davis / Impound
  { coords = vector3(-1200.0, -900.0, 12.0), radius = 900.0, limit = 60 },   -- Vespucci / Del Perro
  { coords = vector3(-300.0, -2000.0, 20.0), radius = 1200.0, limit = 80 },  -- Periferia industrial
  { coords = vector3(1600.0, -1000.0, 60.0), radius = 2000.0, limit = 100 }, -- Vias rápidas sul/este
  { coords = vector3(-900.0, -1600.0, 13.0), radius = 1500.0, limit = 100 }, -- LSIA anel viário
  { coords = vector3(1400.0, 4500.0, 40.0), radius = 2000.0, limit = 90 },   -- Rural Blaine
  { coords = vector3(-1700.0, -300.0, 50.0), radius = 1500.0, limit = 120 }, -- Autoestrada oeste
}
