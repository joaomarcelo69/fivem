-- Safe obtain QBCore/GetCoreObject (avoid startup race)
local function safeGetQBCore()
    if exports and exports['qb-core'] and type(exports['qb-core'].GetCoreObject) == 'function' then
        return exports['qb-core']:GetCoreObject()
    end
    if type(GetCoreObject) == 'function' then
        return GetCoreObject()
    end
    if Global and type(Global.GetCoreObject) == 'function' then
        return Global.GetCoreObject()
    end
    if _G and _G.GetCoreObject then
        return _G.GetCoreObject()
    end
    return { Functions = {}, Commands = {}, Shared = { Items = {} } }
end

-- Abrir painel pt-housing a partir da app "Propriedades" do telemóvel
RegisterNUICallback('OpenPtHousing', function(_, cb)
    QB.Phone.Functions.Close()
    SetTimeout(150, function()
        ExecuteCommand('housing')
    end)
    cb(true)
end)

-- Orders (Encomendas) app: inicializar e sincronizar
RegisterNUICallback('PtOrdersInit', function(_, cb)
    local src = PlayerId()
    QBCore.Functions.TriggerCallback('pt-commerce:orders:listMine', function(list)
        QBCore.Functions.TriggerCallback('pt-commerce:mailbox:get', function(mb)
            SendNUIMessage({ action = 'orders:init', orders = list or {}, mailbox = mb })
        end)
    end)
    cb(true)
end)

RegisterNUICallback('PtOrdersRefresh', function(_, cb)
    QBCore.Functions.TriggerCallback('pt-commerce:orders:listMine', function(list)
        QBCore.Functions.TriggerCallback('pt-commerce:mailbox:get', function(mb)
            SendNUIMessage({ action = 'orders:init', orders = list or {}, mailbox = mb })
        end)
...
RegisterNUICallback('PtOrdersSetMailbox', function(_, cb)
...
RegisterNUICallback('PtOrdersPickup', function(_, cb)
...
-- Functions
-- Integração: refresh de badges de apps por pedido do servidor
RegisterNetEvent('qb-phone:client:RefreshAppAlerts', function()
